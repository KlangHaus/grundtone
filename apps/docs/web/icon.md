...
With the default `gt` prefix:

```html
<svg class="gt-icon gt-icon--lg">...</svg>
```

With a custom `kh` prefix:

```html
<svg class="kh-icon kh-icon--lg">...</svg>
```
...
Pair icon classes with button elements:
...
All custom icons in `@grundtone/core` follow these conventions:

- ViewBox: `0 0 24 24`
- Stroke-based: `fill="none" stroke="currentColor" stroke-width="1.5"`
- Kebab-case filenames: `my-icon.svg`
