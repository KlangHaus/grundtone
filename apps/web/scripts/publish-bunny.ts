/**
 * Sync `.output/public/` (the statically generated Nuxt landing site) to
 * Bunny Edge Storage, per the locked grundtone.com architecture:
 * apex `grundtone.com` = Bunny Edge Storage + Pull Zone with Flatten-record
 * (see `infra/docs/BUNNY-SETUP.md` + `infra/docs/MASTER-PLAN.md` #3).
 * `studio.grundtone.com` serves the same output's `/studio` route via an
 * edge rule/redirect until it warrants its own build ([infra] call).
 *
 * Same sync semantics as `apps/docs/scripts/publish-bunny.ts` (mutable site,
 * unlike packages/email's immutable versioned artifacts):
 ...
 *      remote-only paths).
 *
 * Requires BUNNY_WEB_STORAGE_ZONE + BUNNY_WEB_STORAGE_API_KEY. Optional:
 * BUNNY_WEB_STORAGE_REGION, BUNNY_WEB_STORAGE_PATH_PREFIX. Skips (exit 0,
...
const distRoot = resolve(here, '../.output/public');

const zone = process.env.BUNNY_WEB_STORAGE_ZONE;
const apiKey = process.env.BUNNY_WEB_STORAGE_API_KEY;
const region = process.env.BUNNY_WEB_STORAGE_REGION?.trim();
const prefix = process.env.BUNNY_WEB_STORAGE_PATH_PREFIX?.trim().replace(
  /^\/+|\/+$/g,
  '',
);
...
if (sentryEnabled) {
  Sentry.init({
    ...
    tags: { script: 'publish-bunny', package: '@grundtone/web' },
  });
} else {
  console.warn(
...
if (!zone || !apiKey) {
  console.warn(
    'publish-bunny: BUNNY_WEB_STORAGE_ZONE / BUNNY_WEB_STORAGE_API_KEY not set — skipping deploy. ' +
      'grundtone.com stays unpublished until the Bunny zone is provisioned (see infra/docs/BUNNY-SETUP.md, [infra]).',
  );
  process.exit(0);
}
...
async function main() {
  ...
  if (files.length === 0) {
    throw new Error(
      'publish-bunny: .output/public is empty — run the docs build first',
    );
  }
  console.log(
  ...
  console.log(
    `Published grundtone.com (${files.length} files) to zone "${zone}".`,
  );
}
