...
describe('OverflowMenu', () => {
  it('renders trigger button', () => {
    ...
    expect(wrapper.find('.overflow-menu__trigger').exists()).toBe(true);
  });

  it('panel is hidden by default', () => {
    const wrapper = mountMenu();
    expect(wrapper.find('.overflow-menu__panel').exists()).toBe(false);
  });

  it('opens on trigger click', async () => {
    const wrapper = mountMenu();
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    expect(wrapper.find('.overflow-menu__panel').exists()).toBe(true);
    expect(wrapper.find('.overflow-menu--open').exists()).toBe(true);
  });

  it('renders all items when open', async () => {
    const wrapper = mountMenu();
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    expect(wrapper.findAll('.overflow-menu__link')).toHaveLength(3);
  });

  it('renders item labels', async () => {
    const wrapper = mountMenu();
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    const links = wrapper.findAll('.overflow-menu__link');
  ...
  it('emits select on item click', async () => {
    const wrapper = mountMenu();
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    await wrapper.findAll('.overflow-menu__link')[0].trigger('click');
  ...
  it('applies danger class', async () => {
    const wrapper = mountMenu();
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    expect(wrapper.findAll('.overflow-menu__link')[2].classes()).toContain(
      'overflow-menu__link--danger',
    );
  });
  ...
  it('applies active class', async () => {
    ...
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    expect(wrapper.findAll('.overflow-menu__link')[0].classes()).toContain(
      'overflow-menu__link--active',
    );
  });
  ...
  it('applies disabled class and aria-disabled', async () => {
    ...
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    const link = wrapper.find('.overflow-menu__link');
    expect(link.classes()).toContain('overflow-menu__link--disabled');
  ...
  it('does not emit select for disabled items', async () => {
    ...
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    await wrapper.find('.overflow-menu__link').trigger('click');
    expect(wrapper.emitted('select')).toBeFalsy();
  });

  it('has aria-haspopup and aria-expanded', () => {
    const wrapper = mountMenu();
    const trigger = wrapper.find('.overflow-menu__trigger');
  ...
  it('renders custom label on trigger', () => {
    ...
    expect(wrapper.find('.overflow-menu__trigger').text()).toBe('Sortér');
  });
  ...
  it('applies bordered class when label is provided', () => {
    const wrapper = mountMenu({ label: 'Sortér' });
    expect(wrapper.find('.overflow-menu--bordered').exists()).toBe(true);
  });
  ...
  it('renders links as <a> when href provided', async () => {
    ...
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    expect(wrapper.find('.overflow-menu__link').element.tagName).toBe('A');
  });
  ...
  it('closes on Escape key', async () => {
    ...
    await wrapper.find('.overflow-menu__trigger').trigger('click');
    expect(wrapper.find('.overflow-menu__panel').exists()).toBe(true);
    await wrapper.find('.overflow-menu').trigger('keydown', { key: 'Escape' });
    expect(wrapper.find('.overflow-menu__panel').exists()).toBe(false);
  });
});
